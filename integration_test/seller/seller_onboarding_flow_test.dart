...
void main() {
  ...
  testWidgets('seller lands on seller shell and reaches create listing', (
    tester,
  ) async {
    ...
    await tester.tap(find.byKey(const Key('seller-dashboard-create-listing')));
    await tester.pumpAndSettle();

    expect(find.byType(ListingFormScreen), findsOneWidget);
    expect(find.byKey(const Key('listing-title-field')), findsOneWidget);
    expect(_findDropdownField('listing-category-field-'), findsOneWidget);
    expect(_findDropdownField('listing-make-field-'), findsOneWidget);
  });
}
...
Finder _findDropdownField(String keyPrefix) {
  return find.byWidgetPredicate(
    ...
    description: 'dropdown key prefix $keyPrefix',
  );
}
